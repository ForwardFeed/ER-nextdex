import { readAbilities } from "./proto_utils.js"
...
export function getAbilities(gameData: GameData) {
    const abilities = readAbilities()
    gameData.abilities = new Map(abilities.ability.map<[string, Ability]>(it => [AbilityEnum[it.id], {
        name: it.name,
        desc: it.description,
    }]))
}
