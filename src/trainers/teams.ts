...
const executionMap: {
  ...
  pokemon: (line, context) => {
    ...
    if (line.match(".species")) {
      context.currentPokemon.specie = regexGrabStr(
        line.replace(/\s/g, ""),
        /(?<==)\w+/
      );
    } else if (line.match(".heldItem")) {
      context.currentPokemon.item = regexGrabStr(
        line.replace(/\s/g, ""),
        /(?<==)\w+/
      );
    } else if (line.match(".ability")) {
      context.currentPokemon.ability = regexGrabNum(
        line.replace(/\s/g, ""),
        /(?<==)\d+/,
        0
      );
      ...
      context.currentPokemon.ivs = regexGrabStr(
        line.replace(/\s/g, ""),
        /(?<=\{)[^}]+/
      )
        .split(",")
      ...
      context.currentPokemon.evs = regexGrabStr(
        line.replace(/\s/g, ""),
        /(?<=\{)[^}]+/
      )
        .split(",")
      ...
      context.currentPokemon.nature = regexGrabStr(
        line.replace(/\s/g, ""),
        /(?<==)\w+/
      );
    } else if (line.match(".moves")) {
      context.currentPokemon.moves = regexGrabStr(
        line.replace(/\s/g, ""),
        /(?<==)[\w,]+/,
        ""
      )
