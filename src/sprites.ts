...
const executionMap: {
  ...
  ptr: (line, context) => {
    if (line.match("u32 gMonFrontPic")) {
      ...
      const path = regexGrabStr(line, /(?<=")[^"]+/).replace(
        /[^/]+lz/,
        "front.png"
      );
...
function getSprite(
  ...
  if (!species.visualsOr.value)
    return { front: "", pal: "", shiny: "", back: "" };
  if (species.visualsOr.value.front)
    return {
      front: "graphics/pokemon/" + species.visualsOr.value.front.path + ".png",
      back: "graphics/pokemon/" + species.visualsOr.value.back.path + ".png",
    ...
    getMulFilesData(
      ...
      .then((spriteData) => {
        ...
        spriteResult.spritesPath.forEach((val, key) => {
          ...
          const inFileSubstitutePath = inFilePath.replace(
            "front.png",
            "anim_front.png"
          );
          ...
          try {
            ...
            console.warn(
              `Tried to copy ${inFilePath} to ${outFilePath} error: ${e}`
            );
          }
          ...
          try {
            if (existsSync(paletteFile)) {
              copyFileSync(
                paletteFile,
                join(output_dir_palette, outPaletteFile)
              );
            } else {
              let end: string | RegExpMatchArray =
                paletteFile.match(/(?<=pokemon\/).*/);
              if (!end) {
            ...
            console.warn(
              `Tried to copy ${paletteFile} to ${outPaletteFile} error: ${e}`
            );
          }
          // copy the shiny palette too
          const paletteFileShiny = inFilePath.replace("front.png", "shiny.pal");
          const outPaletteFileShiny =
            "shiny_" + outFileName.replace("png", "pal");
          try {
            if (existsSync(paletteFileShiny)) {
              copyFileSync(
                paletteFileShiny,
                join(output_dir_palette, outPaletteFileShiny)
              );
            } else {
              let end: string | RegExpMatchArray =
                paletteFileShiny.match(/(?<=pokemon\/).*/);
              if (!end) {
            ...
            console.warn(
              `Tried to copy ${paletteFileShiny} to ${paletteFileShiny} error: ${e}`
            );
          }
        });
        resolve(null);
