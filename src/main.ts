...
import { SpeciesEnumSchema, SpeciesEnum } from "./gen/SpeciesEnum_pb.js";
import { Species as ProtoSpecies } from "./gen/SpeciesList_pb.js";
...
export function main(
  ...
  optionsValues: ParsedValues
) {
  const rootPrj = optionsValues.inputPath
    ? optionsValues.inputPath
    : configuration.project_root;
  const outputDir = optionsValues.redirectData
    ? Path.join("./static/js/data/")
    : Path.join("./out/");
  ...
  const outputAdditionnal = Path.join(
    outputDir,
    `additional${outputVersion}.json`
  );
  getFileData(Path.join(rootPrj, "include/global.h"), {
    filterComments: true,
    filterMacros: true,
    macros: new Map(),
  })
    .then((global_h) => {
      ...
      if (optionsValues.comparify.length) {
        ...
        try {
          Sprites.getSprites(rootPrj, OUTPUT_SPRITES, OUTPUT_PALETTES, outputDirPaletteData);
        } catch (err) {
          console.error("error while trying to catch sprites " + err);
        }
      } else {
        const promiseArray: Array<Promise<unknown>> = [];
        Species.getSpecies(gameData);
        Moves.getMoves(gameData);
        Abilities.getAbilities(gameData);
        promiseArray.push(Locations.getLocations(rootPrj, gameData));
        Trainers.getTrainers(gameData);
        promiseArray.push(ScriptedData.parse(rootPrj, gameData));
        BattleItems.getItems(gameData);
        gameData.movesInternalID = new Map(
          MoveEnumSchema.values.map((it) => [it.name, it.number])
        );
        gameData.battleItemsInternalID = new Map(
          ItemEnumSchema.values.map((it) => [it.name, it.number])
        );
        gameData.speciesInternalID = new Map(
          SpeciesEnumSchema.values.map((it) => [it.name, it.number])
        );
        gameData.abilitiesInternalID = new Map(
          AbilityEnumSchema.values.map((it) => [it.name, it.number])
        );
        ...
        Promise.allSettled(promiseArray)
          .then((values) => {
            ...
            Additionnal.getAdditionnalData(
              rootPrj,
              outputAdditionnal,
              gameData
            );
