...
function parse(pokeData: string): Specie[] {
  ...
  const speciesNamesResult = SpeciesNames.parse(
    lines,
    pokePokedexResult.fileIterator
  );
  const baseStatsResult = BaseStats.parse(
    lines,
    speciesNamesResult.fileIterator
  );
  const evolutionsResult = Evolutions.parse(
    lines,
    baseStatsResult.fileIterator
  );
  const eggMovesResult = EggMoves.parse(lines, evolutionsResult.fileIterator);
  const levelUpLearnsetsResult = LevelUpLearnSets.parse(
    lines,
    eggMovesResult.fileIterator
  );
  const TMHMLearnsetsResult = TMHMLearnsets.parse(
    lines,
    levelUpLearnsetsResult.fileIterator
  );
  const TutorMovesResult = TutorMoves.parse(
    lines,
    TMHMLearnsetsResult.fileIterator
  );
...
export function getLegacySpecies(
  ROOT_PRJ: string,
...
function createEvoMapping(
  speciesList: SpeciesList
): Map<SpeciesEnum, [Evolution[], string[]]> {
  const evoMap = new Map<SpeciesEnum, [Evolution[], string[]]>(
    speciesList.species.map<[SpeciesEnum, [Evolution[], string[]]]>((it) => [
      it.id,
      [[], []],
    ])
  );
  ...
  for (const species of speciesList.species) {
    ...
    for (const evo of species.evo) {
      evos.push({
        kind: evo.gender
          ? "EVO_LEVEL_" + Species_Gender[evo.gender]
          : "EVO_LEVEL",
    ...
    for (const mega of species.mega) {
      evoMap.get(mega.from)!![0].push({
        kind:
          mega.evoUsing.case === "move"
            ? "EVO_MOVE_MEGA_EVOLUTION"
            : "EVO_MEGA_EVOLUTION",
        specifier:
          mega.evoUsing.case === "move"
            ? MoveEnum[mega.evoUsing.value]
            : ItemEnum[mega.evoUsing.value || ItemEnum.ITEM_NONE],
...
function getBaseSpecies(
  species: Species,
  speciesMap: Map<SpeciesEnum, Species>
): Species_SpeciesDexInfo {
  ...
  switch (species.baseSpeciesInfo.case) {
    ...
    case "formOf":
      return getBaseSpecies(
        speciesMap.get(species.baseSpeciesInfo.value)!!,
        speciesMap
      );
    default:
...
function getLearnsetMon(
  ...
  if (species.learnsetOrRef.value)
    return getLearnsetMon(
      speciesMap.get(species.learnsetOrRef.value)!!,
      speciesMap
    );
  if (species.formShiftOf)
    return getLearnsetMon(speciesMap.get(species.formShiftOf)!!, speciesMap);
  if (species.mega.length)
    return getLearnsetMon(speciesMap.get(species.mega[0].from)!!, speciesMap);
  if (species.primal.length)
    return getLearnsetMon(speciesMap.get(species.primal[0].from)!!, speciesMap);
...
function getUniversalTutors(
  type: Species_Learnset_UniversalTutors,
...
export function toSpeciesMap(
  speciesList: SpeciesList
): Map<SpeciesEnum, Species> {
...
export function getSpecies(gameData: GameData) {
  ...
  gameData.speciesMap = toSpeciesMap(speciesList);

  gameData.species = [];
  for (const species of speciesList.species) {
    if (
      species.id === SpeciesEnum.SPECIES_EGG ||
      species.randomizerBanned === Species_RandomizeBanned.SPECIES_HIDDEN
    )
      continue;
    let baseSpeciesInfo;
    try {
      baseSpeciesInfo = getBaseSpecies(species, gameData.speciesMap);
    } catch (e) {
      console.warn(e);
      continue;
    }

    const learnsetSpecies = getLearnsetMon(species, gameData.speciesMap);
    const learnset =
      (learnsetSpecies.learnsetOrRef.value as Species_Learnset) ||
    ...
    if (!name && species.mega.length) {
      name = `${baseSpeciesInfo.name} ${Xtox(
        "",
        Species_MegaEvolution_MegaType[species.mega[0].type].replace(
          "_UNSPECIFIED",
          ""
        )
      )}`;
    ...
    if (!name && species.primal.length) {
      name = `${baseSpeciesInfo.name} ${Xtox(
        "",
        Species_PrimalEvolution_PrimalType[species.primal[0].type]
      )}`;
    }
    ...
    gameData.species.push({
      ...
      baseStats: {
        ...
        types: [
          Xtox("", Type[species.type]),
          Xtox("", Type[species.type2 ? species.type2 : species.type]),
        ],
        catchRate: 0,
        expYield: 0,
        evYield_HP: 0,
        ...
        items: [],
        genderRatio:
          species.gender.case === "genderless"
            ? 255
            : Math.floor(254 * (species.gender.value || 0)),
        eggCycles: 0,
        friendship: 0,
        growthRate: "",
        eggGroup: [baseSpeciesInfo.eggGroup, baseSpeciesInfo.eggGroup2]
          .filter((it) => it != EggGroup.EGG_GROUP_NONE)
          .map((it) => "EGG_GROUP_" + EggGroup[it]),
      ...
      eggMoves: [],
      levelUpMoves: learnset.level.flatMap((levelGroup) =>
        levelGroup.move.map<LevelUpLearnSets.LevelUpMove>((it) => {
          return { level: levelGroup.level, move: MoveEnum[it] };
        })
      ),
      tutorMoves: learnset.tutor
        .map((it) => MoveEnum[it])
        .concat(
          ...getUniversalTutors(
            learnset.universalTutors,
            species.gender.case === "genderless"
          )
        ),
      TMHMMoves: [],
